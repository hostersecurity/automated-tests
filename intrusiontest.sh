...
function	runtests()
{
	...
	runtestcommand $1 "cat /tmp/shadow 2>&1" $2 "test_cat_shadow"
	runtestcommand $1 "cat /tmp/passwd 2>&1" $2 "test_cat_passwd"
	runtestcommand $1 "uname -a" $2 "test_uname"
	runtestcommand $1 "whoami" $2 "test_whoami"
	runtestcommand $1 "groups $(whoami)" $2 "test_my_groups"
